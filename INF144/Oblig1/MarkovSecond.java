...
public class MarkovSecond {

    // Random generator and HashMap
    private static Random rnd = new Random();
    private static HashMap<String, MarkovBagSquared> mChain = new HashMap<String, MarkovBagSquared>();
    ...
    public static void addText(String[] phrase) {
        ...
        for(int i = 0; i < phrase.length; i ++) {
            ...
            if(!mChain.containsKey(phrase[i])) {
                mChain.put(phrase[i], new MarkovBagSquared());
            }
            
            else if(i != phrase.length - 1){
                MarkovBagSquared bag = mChain.get(phrase[i]);
                bag.add(phrase[i+1]);
